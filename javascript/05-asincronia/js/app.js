...
async function cargarParalelo() {
  ...
  try {
    ...
    resultadosPromesas.forEach((r) => {
      mostrarLog(`✓ ${r.nombre} cargado en ${formatearTiempo(r.tiempo)}`, 'success');
    });
...
function mostrarComparativa() {
  if (tiempoSecuencial > 0 && tiempoParalelo > 0) {
    ...
    resultados.innerHTML = `
      ...
      <p><strong>Secuencial:</strong> ${formatearTiempo(tiempoSecuencial)}</p>
      <p><strong>Paralelo:</strong> ${formatearTiempo(tiempoParalelo)}</p>
...
async function simularError() {
  ...
  try {
    ...
    mostrarLogError('ℹ️ Error manejado correctamente con try/catch', 'info');
  }
}

async function fetchConReintentos(nombre, intentos = 3) {
  mostrarLogError(`🔄 Iniciando ${intentos} intentos...`, 'info');

  for (let i = 0; i < intentos; i++) {
    try {
      mostrarLogError(`⏳ Intento ${i + 1}`, 'info');
